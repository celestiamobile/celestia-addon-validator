...
public final class Validator {
    ...
    private func validateDirectory(_ path: String) async throws -> ItemOperation {
        ...
        if !modifyingExistingAddon {
            ...
            let (relatedObjectPaths, _) = try await validateAddonContents(location: .local(url: addonURL))
        ...
        if let addonURL {
            (relatedObjectPaths, needsUpdate) = try await validateAddonContents(location: .local(url: addonURL))
        } else {
            (relatedObjectPaths, needsUpdate) = try await validateAddonContents(location: .remote(id: idRequirement))
        }
    ...
    private func validateAddonContents(location: AddonLocation) async throws -> ([String], Bool) {
        ...
        return (Array(Set(objectPaths)), true)
    }
